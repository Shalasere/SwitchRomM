...
- [H] Resume integrity (`source/manifest.cpp`, `source/downloader.cpp`): contiguity enforced in planResume; validation still size-only (hashes TODO; server doesn’t provide), temp dirs named by truncated title can collide; final names can overwrite. **Fix**: add stronger validation (hash when feasible), include rom/file IDs in temp/final names or collision-safe scheme, size-consistent progress (use Content-Length if present).
